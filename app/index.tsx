...
export default function Index() {
  ...
  // Mémorisation des styles pour éviter de les recréer à chaque frappe au clavier
  const styles = useMemo(() => StyleSheet.create({
    container: { flex: 1, backgroundColor: colors.background },
    keyboardAvoid: { flex: 1 },
    scrollContent: { flexGrow: 1, padding: 24, justifyContent: 'center' },
  ...
  useEffect(() => {
    ...
    // Ajout d'un bloc catch pour éviter un blocage infini en cas de problème réseau
  ...
  const handleAuth = useCallback(async () => {
    ...
    try {
      if (isSignUpMode) {
        ...
        // Pas besoin de router.replace ici car le listener onAuthStateChange s'en charge
      }
    } catch (err: any) {
      let errorMessage = 'Une erreur est survenue.';
...
import { useRouter, useRootNavigationState } from 'expo-router';
import { supabase } from '../lib/supabase';
import AsyncStorage from '@react-native-async-storage/async-storage'; // AJOUT IMPORTANT
...
export default function Index() {
  ...
  // LE NOUVEAU USE-EFFECT QUI GÈRE LE HORS LIGNE
  useEffect(() => {
    if (!rootNavigationState?.key) return;

    let isMounted = true;

    const checkSessionOfflineFriendly = async () => {
      try {
        // Course entre la requête Supabase et un minuteur de 2 secondes
        const sessionPromise = supabase.auth.getSession();
        const timeoutPromise = new Promise((_, reject) =>
          setTimeout(() => reject(new Error('TIMEOUT')), 2000)
        );

        // On attend que l'un des deux termine en premier
        ...
        // Si erreur réseau OU timeout atteint (mode offline détecté)
        if (!isMounted) return;

        try {
          // On force la lecture directe dans AsyncStorage
          // ATTENTION: Assurez-vous que l'ID correspond bien à votre URL Supabase
          const localData = await AsyncStorage.getItem('sb-tuciyiawyawrhifpjmmn-auth-token');
          
          if (localData) {
            const parsedSession = JSON.parse(localData);
            if (parsedSession?.access_token) {
              console.log("Session locale trouvée, accès accordé (Mode Offline)");
              router.replace('/(tabs)');
            } else {
              setLoading(false);
            }
          ...
          console.log("Erreur de lecture du cache:", cacheError);
          setLoading(false);
        }
      }
    };

    checkSessionOfflineFriendly();

    // On garde le listener pour les changements d'état futurs (connexion/déconnexion)
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event, session) => {
        if (!isMounted) return;
        if (event === 'SIGNED_IN' && session) {
          ...
          // On évite de forcer setLoading(false) ici immédiatement lors de l'initialisation
          // car checkSessionOfflineFriendly s'en charge avec le fallback.
        }
      }
    );
...
export default function Index() {
  ...
  // LE NOUVEAU USE-EFFECT QUI GÈRE LE HORS LIGNE PERMANENT
  useEffect(() => {
    if (!rootNavigationState?.key) return;
    ...
    const checkSessionOfflineFriendly = async () => {
      try {
        // 1. Tenter la vérification Supabase avec timeout de 2 secondes
        const sessionPromise = supabase.auth.getSession();
        const timeoutPromise = new Promise((_, reject) =>
          setTimeout(() => reject(new Error('TIMEOUT')), 2000)
        );
        ...
        if (response?.data?.session) {
          // Connexion en ligne réussie : on enregistre/maintient le drapeau local
          await AsyncStorage.setItem('@is_logged_in', 'true');
          router.replace('/(tabs)');
          return;
        }
      } catch (error) {
        console.log("Mode hors ligne ou délai dépassé. Vérification du statut local...");
      }

      // 2. Si hors ligne OU si timeout : on vérifie notre drapeau local permanent
      try {
        ...
        if (isLocallyLoggedIn === 'true') {
          console.log("Accès hors ligne accordé via statut local permanent.");
          router.replace('/(tabs)');
        } else {
          setLoading(false);
        }
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event, session) => {
        if (!isMounted) return;
        if (event === 'SIGNED_IN' && session) {
          // Maintien du drapeau si la session est restaurée en ligne
          await AsyncStorage.setItem('@is_logged_in', 'true');
          router.replace('/(tabs)');
        }
      }
  ...
  const handleAuth = useCallback(async () => {
    ...
    try {
      if (isSignUpMode) {
        ...
        // Enregistrer le statut permanent lors d'une connexion réussie
        await AsyncStorage.setItem('@is_logged_in', 'true');
  ...
  useEffect(() => {
    ...
    const checkSessionInstantly = async () => {
      try {
        ...
        if (isLocallyLoggedIn === 'true') {
          // Redirection directe vers les tabs
          router.replace('/(tabs)');
          // Attendre que la navigation soit traitée avant de cacher le Splash Screen
          setTimeout(() => {
